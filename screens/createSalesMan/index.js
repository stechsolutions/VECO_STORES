...
import { FlatList, StyleSheet, Text, TouchableOpacity, View, ScrollView, Modal, PermissionsAndroid, Alert } from 'react-native';
...
const index = ({ navigation }) => {
    ...
    const create = async () => {
        ...
        auth().createUserWithEmailAndPassword(email, password).then((res) => {
            storage().ref(`salesMan/${res.user.uid}/${Date.now()}`)
                .put(profileBlob)
                .on(
                    'state_changed',
                    () => { },
                    () => { },
                    (imgResponse) => imgResponse.ref.getDownloadURL().then((url) => {
                        console.log(url, 'url');
                        obj.image = url;
                        return firestore().collection('salesMan').doc(res.user.uid).set(obj);
                    }))
        })
            .then(() => {
                setLoading(false);
                ...
                Alert.alert(
                    "Sales Man Created",
                    "Sales Man has been created successfully",
                    [
    ...
    const getSalesMan = async () => {
        const store = JSON.parse(await AsyncStorage.getItem('store'));
        firestore().collection('salesMan').where('storeId', '==', store.storeId)
            .onSnapshot((snapshot) => {
                var salesMen = [];
                snapshot.forEach((e => {
                    salesMen.push({ ...e.data(), userId: e.id });
                }))
                ...
                <ScrollView style={{ flex: 1, padding: 10 }}>
                    ...
                    <View style={styles.inputView}>
                        <AppPhotoInput
                            ...
                            placeHolder={location ? `${location.latitude}, ${location.longitude}` : "Longitude/Latitude"}
                            onPress={() => {
                                checkPermission();
                            }}
                        />
                    ...
                    <View style={styles.subContainer}>
                        <AppButton
                            disabled={!fullName || !phone ||
                                !image || !location || !email || !password || !termsAccepted}
                            loading={loading}
                            onPress={create}
                            color={colors.primary}
                            style={styles.btn}
                            title="Create Sales Man" />
                    </View>
                </ScrollView>
