...
const index = ({navigation}) => {
      ...
      .on('state_changed',
       ...
       ((imageRes) => {
        imageRes.ref.getDownloadURL()
          .then(url => {
            const promotion = { name, description, imageUri: url, selectedProduct, startDate, endDate,promotionType:type };
            firestore().collection('vendorStores').doc(store.storeId).collection('promotions').add(promotion)
              .then(res => {
                setLoading(false);
  ...
  const getProducts = async () => {
    var store = JSON.parse(await AsyncStorage.getItem('store'));
    firestore().collection('vendorStores').doc(store.storeId).collection('products').get()
    .then(res=>{
      var products = [];
      res.forEach(e=>{
        products.push({...e.data(),productId:e.id,label:e.data().productName});
      })
  ...
  return (
    <Screen style={styles.container}>
      <ScrollView style={{ padding: 10 }}>
        ...
        <AppTextInput style={styles.mVertical} placeHolder="Name" value={name} onChangeText={(txt) => setName(txt)} />
        <AppMultiLineInput style={styles.mVertical} placeholder="Description" value={description} onChangeText={(txt) => setDescription(txt)} />
        <AppPicker onSelectItem={(txt)=>{setType(txt.label)}} items={types} style={styles.mVertical} title={type ? type : "Promotion Type"} />
        ...
        <AppButton
        disabled={!selectedProduct || !imageUri || !name || !description || !type}
        loading={loading}
          title="CREATE"
