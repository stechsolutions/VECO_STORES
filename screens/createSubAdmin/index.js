...
const index = ({ navigation }) => {
    ...
    const create = async () => {
        ...
        auth().createUserWithEmailAndPassword(email, password).then((res) => {
            storage().ref(`subAdmins/${res.user.uid}/${Date.now()}`)
                .put(profileBlob)
                .on(
                    'state_changed',
                    () => { },
                    ...
                    (imgResponse) => imgResponse.ref.getDownloadURL().then((url) => {
                        console.log(url, 'url');
                        obj.image = url;
                        return firestore().collection('subAdmins').doc(res.user.uid).set(obj);
                    }))
        })
            .then(() => {
                setLoading(false);
    ...
    const getSubAdmins = async () => {
        const store = JSON.parse(await AsyncStorage.getItem('store'));
        firestore().collection('subAdmins').where('storeId', '==', store.storeId)
            .onSnapshot((snapshot) => {
                var subAdmins = [];
                snapshot.forEach((e => {
                ...
                <ScrollView style={{ flex: 1, padding: 10 }}>
                    ...
                    <View style={styles.subContainer}>
                        <AppButton
                            ...
                            onPress={create}
                            color={colors.primary}
                            style={styles.btn} title="Create SubAdmin" />
                    </View>
                </ScrollView>
