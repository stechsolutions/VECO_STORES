...
const index = ({navigation}) => {
    ...
    const create = async () => {
        ...
        auth().createUserWithEmailAndPassword(email, password).then((res) => {
            storage().ref(`dispatchers/${res.user.uid}/${Date.now()}`)
                .put(profileBlob)
                .on(
                    'state_changed',
                    () => { },
                    ...
                    (imgResponse) => imgResponse.ref.getDownloadURL().then((url) => {
                        console.log(url, 'url');
                        obj.image = url;
                        return firestore().collection('dispatchers').doc(res.user.uid).set(obj);
                    }))
        })
            .then(() => {
                setLoading(false);
                ...
                Alert.alert(
                    "Dispatcher Created",
                    "Dispatcher has been created successfully",
                    [
    ...
    const getDispatchers = async () => {
        const store = JSON.parse(await AsyncStorage.getItem('store'));
        firestore().collection('dispatchers').where('storeId', '==', store.storeId)
            .onSnapshot((snapshot) => {
                var dispatchers = [];
                snapshot.forEach((e => {
                    dispatchers.push({ ...e.data(), userId: e.id });
                }))
