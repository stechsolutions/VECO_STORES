...
import { Image, LogBox, ScrollView, StyleSheet, Text, Alert, View, ActivityIndicator } from 'react-native';
...
const index = (props) => {
  ...
  const getProduct = async () => {
    ...
    firestore().collection('store').doc(store.storeId).collection('product').doc(productId).get()
      .then(res => {
  ...
  const editProducts = async () => {
    ...
    const product = { productType, productName, totalAvailable, priceType, color, size, shortDescription, longDescription };
    var store = JSON.parse(await AsyncStorage.getItem('store'));
    console.log("This is the store inside upload: ", store);
    firestore().collection('store').doc(store.storeId).collection('product').doc(props.route.params.productId).update(product)
      .then(() => {
  ...
  const deleteProduct = async () => {
    ...
    var store = JSON.parse(await AsyncStorage.getItem('store'));
    console.log("This is the store inside upload: ", store);
    firestore().collection('store').doc(store.storeId).collection('product').doc(props.route.params.productId).delete(product)
      .then(() => {
        ...
        product ?
          <ScrollView style={{ flex: 1, padding: 10 }}>
            <View style={styles.userContainer}>
              <Image
                style={styles.image}
                source={image ? { uri: image } : require('../../assets/images/Spray.jpg')}
              />
              <Text style={styles.title}>{productName}</Text>
            </View>
            <AppPicker style={styles.mVertical} title={productType ? productType : "Product Type"}
              value={productTypes} onSelectItem={(txt) => { setProductType(txt) }} />
            <AppTextInput
            ...
            <View
              ...
              <AppTextInput
                ...
                value={totalAvailable} onChangeText={(txt) => { setTotalAvaliable(txt) }}
              />
              <AppPicker
                items={priceTypes}
                style={[
                ...
                value={priceType} onSelectItem={(txt) => { setPriceType(txt.label) }}
              />
            </View>
            <View
              style={[
